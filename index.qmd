...
X <- make_model_matrix(full_model$call$formula)
...
> `citric_acid`, `residual_sugar`, `chlorides`, `total_sulfur_dioxide`,
> `density`, `pH`, `sulphates`, `alcohol` and `type`
...
```{r}
lasso_vars <- colnames(lasso_coef)[-1]

lasso_formula <- make_formula <- function(lasso_vars){
  as.formula(paste("quality ~ .", paste(lasso_vars, collapse = "+")))
}
```
...
```{r}
ridge_vars <- colnames(ridge_coef)[-1]

ridge_formula <- make_formula <- function(ridge_vars){
  as.formula(paste("quality ~ ", paste(ridge_vars, collapse = "+")))
}
```
...
> resulted in a model with three predictors, while LASSO and ridge
> regression selected different sets of predictors. The LASSO model had
> a single non-zero coefficient, while the ridge model had non-zero
> coefficients for all predictors, but they were smaller in magnitude
...
>     criteria.
>
> -   In terms of which method is more reliable. Exhaustive search
>     through all possible models is not practical for datasets with
>     many variables, so some form of model selection is necessary.
>     Because of this LASSO or Ridge regression would be good options.
>
> -   If we had a dataset with 10,000 columns, exhaustive search through
>     all possible models would be computationally infeasibl. In this
>     case, LASSO or Ridge regression could be good options because they
>     can handle high-dimensional datasets and automatically shrink the
>     coefficients of irrelevant predictors towards zero.
